...
class Mp3Player(Node):
    def __init__(self):
        ...
        self.reply_path = "/home/nvidia/ros2_ws/src/pkg_spk/pkg_spk/reply.wav"  # WAV 포맷으로 변경
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    def text2speech(self, text):
        ...
        data = {
            ...
            "speed": "0",       # 속도 (-5 ~ 5)  
            "pitch": "0",       # 음높이 (-5 ~ 5)
            "format": "wav",    # 출력 포맷 (mp3, wav, ogg) - WAV로 변경하여 변환 과정 생략
            "text": cleaned_text
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 🆕 WAV 변환 과정 생략 - 이미 WAV 포맷으로 받았으므로 바로 STT 처리
                self.get_logger().info("✅ WAV 변환 과정 생략 - 이미 WAV 포맷으로 수신됨")
                
                # 🆕 STT 타임스탬프 추출 (직접 WAV 파일 사용)
                stt_timestamps = self.extract_word_timestamps(self.reply_path, cleaned_text)
    ...
    def play_tts_with_spectrum(self, file_path):
        ...
        try:
            ...
            if file_path.endswith('.wav'):
                temp_wav = file_path
                self.get_logger().info("✅ WAV 포맷 직접 사용 - 변환 과정 생략")
            else:
                # MP3인 경우에만 변환 (호환성 유지)
                sound = AudioSegment.from_file(file_path, format="mp3")
                sound = self.match_target_amplitude(sound, -14.0)
